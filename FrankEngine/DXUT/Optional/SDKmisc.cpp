...
HRESULT CDXUTTextHelper::DrawFormattedTextLine( const WCHAR* strMsg, ... )
{
    ...
    return DrawTextLine( strBuffer, false, true );
}


//--------------------------------------------------------------------------------------
HRESULT CDXUTTextHelper::DrawTextLine( const WCHAR* strMsg, bool advanceUp, bool outline )
{
    ...
    if( m_pFont9 )
	{
		if (outline)
		{
			// just render it 4 times in each direction to make an outline
			SetRect( &rc, m_pt.x+1, m_pt.y+0, 0, 0 ); 
			hr = m_pFont9->DrawText( m_pSprite9, strMsg, -1, &rc, DT_NOCLIP, D3DXCOLOR(0,0,0,m_clr.a) );
			SetRect( &rc, m_pt.x-1, m_pt.y+0, 0, 0 ); 
			hr = m_pFont9->DrawText( m_pSprite9, strMsg, -1, &rc, DT_NOCLIP, D3DXCOLOR(0,0,0,m_clr.a) );
			SetRect( &rc, m_pt.x+0, m_pt.y-1, 0, 0 ); 
			hr = m_pFont9->DrawText( m_pSprite9, strMsg, -1, &rc, DT_NOCLIP, D3DXCOLOR(0,0,0,m_clr.a) );
			SetRect( &rc, m_pt.x+0, m_pt.y+1, 0, 0 ); 
			hr = m_pFont9->DrawText( m_pSprite9, strMsg, -1, &rc, DT_NOCLIP, D3DXCOLOR(0,0,0,m_clr.a) );
		}
...
{
	ASSERT(g_render->GetIsInRenderBlock());
