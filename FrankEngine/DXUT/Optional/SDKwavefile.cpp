...
#include <fstream>
#include "../source/core/debugMessage.h"
...
HRESULT CWaveFile::Open( LPWSTR strFileName, WAVEFORMATEX* pwfx, DWORD dwFlags )
{
    ...
    if( m_dwFlags == WAVEFILE_READ )
    {
		...
		WCHAR soundNameWav[MAX_PATH];
		wcsncpy_s(soundNameWav, MAX_PATH, strFileName, _TRUNCATE );
		wcscat_s(soundNameWav, MAX_PATH, L".wav");
		
		// this would be the easy way but we want it to close the file so you can change and reload the assed during development
		//m_hmmio = mmioOpen( soundNameWav, NULL, MMIO_ALLOCBUF | MMIO_READ );
		
		// try read a local wave file from disk
		fstream waveFile(soundNameWav, fstream::in | fstream::binary);
		if (waveFile.is_open())
		{
			// get the file size
			...
			//g_debugMessageSystem.AddFormatted(L"Loaded local sound \"%s\"", soundNameWav);
		}
		else
		{
            HRSRC   hResInfo;
