...
void DXUTRender3DEnvironment9()
{
	...
	static bool needsPresent = false;
	{
		FrankProfilerEntryDefine(L"Total Update Time", Color::Green(), -1000);
		...
		else if(!GetDXUTState().GetRenderingPaused())
		{
			...
			// preset moved to separate block so it won't be included in total frame time
			needsPresent = true;
		}
	}
...
void DXUTAllowShortcutKeys( bool bAllowKeys )
{
    ...
    else
    {
        ...
        {
			// HACK - fix 'GetVersionExA': was declared deprecated, assume recent OS
